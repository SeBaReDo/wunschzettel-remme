...
import {
  ...
  setDoc,
  deleteDoc,
  onSnapshot,import { initializeApp } from "https://www.gstatic.com/firebasejs/10.13.0/firebase-app.js";
import {
  getFirestore,
  doc,
  setDoc,
...
function formatPrice(price) {
  ...
  return price.replace(/\d+(?:[.,]\d+)?/g, (numStr) => {
...
function renderWishes() {
  ...
  wishes.forEach((wish) => {
    ...
    const showProductLink = !!(wish.link && !isReserved);

    if (wish.description || showProductLink) {
      const desc = document.createElement("div");
      desc.className = "wish-desc";
      if (wish.description) {
        desc.appendChild(document.createTextNode(wish.description));
      }
      if (showProductLink) {
        if (wish.description) {
